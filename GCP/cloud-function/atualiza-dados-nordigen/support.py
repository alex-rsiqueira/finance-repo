...
def log_error(
    error, desc_e=None, project_id=PROJECT_ID, dataset_id=DATASET_ID, table_id=None
):
    ...
    log_table = pd.concat(
        [log_table, pd.DataFrame(new_line, index=[0])], ignore_index=True
    )
...
def insert_db(df, table_id, dataset_id, project_id):
    ...
    try:
        ...
        pd.io.gbq.to_gbq(
            df, destination_table=table_ref, if_exists="replace", project_id=project_id
        )
