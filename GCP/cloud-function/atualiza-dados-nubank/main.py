...
from support import download_blob, get_nubank_accounts, read_secret
...
def main(event, context):
    try:
        ...
        nubank_accounts = get_nubank_accounts()

        print("Loop through accounts")
        for row in nubank_accounts:
            print(f"Start ingestion for user: {row['cpf']}")

            # Get credentials
            password = read_secret(row["cpf"])

            # Get certificate file from Cloud Storage
            cert_name = "cert-" + str(row["cpf"])[-5:] + ".p12"
            download_blob(cert_name)

            nu = Nubank()
            nu.authenticate_with_cert(row["cpf"], password, "/tmp/" + cert_name)
            ...
            insert_row = bq_client.insert_rows_from_dataframe(
                table=table, dataframe=df_client
            )
            print("Tabela populada com sucesso: tb_nubank_client")
            ...
            table = bq_client.get_table(dataset)
            insert_row = bq_client.insert_rows_from_dataframe(
                table=table, dataframe=df_credit
            )
            print("Tabela populada com sucesso: tb_nubank_credit")
            ...
            for i in range(0, len(bills)):
                ...
                if bill_state != "future":
                    if bill_state in ("closed", "overdue"):
                        ...
                        bill_href = bills[i]["_links"]["self"]["href"]
                        bill_id = bill_href[::-1][
                            bill_href[::-1].find("sllib/") + 6 : bill_href[::-1].find(
                                "/stnuocca"
                            )
                        ][::-1]
                    else:
                    ...
                    if (
                        int(bill_competence.replace("-", ""))
                        >= int(last_open_bill.replace("-", "")) - 1
                    ):
                        ...
                        dtinsert = datetime.datetime.today().strftime(
                            "%Y-%m-%d %H:%M:%S"
                        )
                        df_bills["dtinsert"] = dtinsert

                        ## Insere registros no BigQuery
                        dataset = bq_client.dataset(dataset_id="raw").table(
                            "tb_nubank_bills"
                        )
                        table = bq_client.get_table(dataset)
                        insert_row = bq_client.insert_rows_from_dataframe(
                            table=table, dataframe=df_bills
                        )
                        print("Tabela populada com sucesso: tb_nubank_bills")

                        bill_details = nu.get_bill_details(bills[i])

                        # Cria dataframe com lançamentos de cada fatura
                        df_bill_statements = pd.DataFrame(
                            bill_details["bill"]["line_items"]
                        )
                        ...
                        df_bill_statements = df_bill_statements.where(
                            pd.notnull(df_bill_statements), None
                        )
                        df_bill_statements = df_bill_statements.astype(object).replace(
                            np.nan, "None"
                        )

                        ## Insere registros no BigQuery
                        dataset = bq_client.dataset(dataset_id="raw").table(
                            "tb_nubank_bill_statements"
                        )
                        table = bq_client.get_table(dataset)
                        insert_row = bq_client.insert_rows_from_dataframe(
                            table=table, dataframe=df_bill_statements
                        )
                        print("Tabela populada com sucesso: tb_nubank_bill_statements")
                ...
                if bill_state == "future":
                    ...
                    df_bills = pd.DataFrame(bills[i]["summary"], index=[0])
                    df_bills = df_bills[
                        ["open_date", "due_date", "total_balance", "minimum_payment"]
                    ]
                    df_bills["bill_id"] = bill_id
                    df_bills["bill_competence"] = bill_competence
                    df_bills["state"] = bill_state
